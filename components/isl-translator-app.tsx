      ...
      <div className="relative flex h-dvh max-h-dvh min-h-0 flex-col overflow-hidden bg-background text-foreground">
        ...
        <div className="relative z-10 flex min-h-0 min-w-0 flex-1 flex-col">
          <header className="z-40 shrink-0 border-b border-white/[0.06] bg-background/55 backdrop-blur-2xl">
            <div className="mx-auto flex w-full max-w-[min(100%,1880px)] items-center justify-between gap-4 px-5 py-3 sm:px-6 lg:px-8">
              <div className="flex min-w-0 flex-1 items-center gap-4">
                <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-primary/40 via-indigo-500/30 to-secondary/30 shadow-[var(--shadow-glow)] ring-1 ring-white/10">
                  <Sparkles className="h-6 w-6 text-primary-foreground" />
                </div>
                <div className="min-w-0">
                  <p className="text-[10px] font-bold uppercase tracking-[0.25em] text-muted-foreground/90">Showcase</p>
          ...
          <main className="mx-auto flex min-h-0 w-full max-w-[min(100%,1880px)] flex-1 flex-col px-5 pb-4 pt-2 sm:px-6 lg:px-8">
            <motion.div
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
              className="mb-2 flex min-w-0 shrink-0 flex-col gap-2.5 sm:flex-row sm:items-center sm:justify-between sm:gap-4"
            >
              <div className="min-w-0">
                <h2 className="text-[11px] font-semibold uppercase tracking-widest text-muted-foreground">Experience</h2>
                <p className="mt-0.5 max-w-2xl text-xs leading-snug text-muted-foreground sm:text-[13px]">
                  Compose · live signer · gloss timeline — one screen.
                </p>
              </div>

              <div className="flex shrink-0 rounded-xl border border-white/[0.08] bg-card/30 p-0.5 shadow-inner backdrop-blur-md">
                <button
                  type="button"
                  onClick={() => setActiveTab('main')}
                  className={cn(
                    'flex items-center gap-2 rounded-lg px-3.5 py-2 text-sm font-semibold transition-all sm:px-4 sm:py-2.5',
                    activeTab === 'main'
            ...
            {processing ? (
              <motion.p
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                className="mb-2 shrink-0 rounded-xl border border-secondary/20 bg-secondary/10 px-4 py-2 text-center text-sm font-medium text-secondary"
              >
                {processing}
              </motion.p>
            ) : null}
            ...
            <div
              className={
                activeTab === 'main'
                  ? 'grid min-h-0 min-w-0 flex-1 grid-cols-1 gap-5 overflow-hidden lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)_minmax(0,1fr)] lg:grid-rows-1 lg:items-stretch lg:justify-items-stretch lg:gap-5'
                  : 'flex min-h-0 flex-1 flex-col gap-4 overflow-hidden'
              }
            >
              {activeTab === 'main' ? (
                <div className="order-2 flex h-full min-h-0 w-full min-w-0 max-w-full max-h-full lg:order-none">
                  <InputPanel
                    value={inputText}
              ...
              <div
                className={
                  activeTab === 'dataset'
                    ...
                    : 'order-1 flex h-full min-h-0 w-full min-w-0 max-w-full max-h-full flex-col gap-4 lg:order-none'
                }
              >
                <AvatarPlayer
              ...
              {activeTab === 'main' ? (
                <div className="order-3 flex h-full min-h-0 w-full min-w-0 max-w-full max-h-full lg:order-none">
                  <GlossDisplay
